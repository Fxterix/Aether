// Copyright 2013 Dolphin Emulator Project / 2026 Aether Emulator Project
...
std::string SerializePath(const std::string& input, bool is_saving) {
    auto result = input;
    StringReplace(result, "%AETHER_ROM_FILE%", g_currentRomPath, is_saving);
    StringReplace(result, "%AETHER_USER_DIR%", GetUserPath(UserPath::UserDir), is_saving);
    return result;
}
